  ...
  <div class="login-wrap">
    <el-form class="login-container">
      ...
      <el-form-item label="身份证号">
        <el-input type="text" v-model="inputDate.idCard" placeholder="身份证号" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="登录密码">
        <el-input type="password"  v-model="inputDate.password" placeholder="登录密码" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
...
methods: {
    ...
    const that=this     
      request.post('/login',{
        idCard:this.inputDate.idCard,
        password:this.inputDate.password
      }).then(function (res){          
        if (res.data.code===200){
          sessionStorage.setItem("token",res.data.data.token)
         console.log("登陆后获得的token是")
          console.log(res.data.data.token)
          store.dispatch('updateUser',res.data.data.user)
          const role=res.data.data.user.role//获取后端权限
          console.log(role)          
          if (role==='admin'){
            //那么进入admin页面 
            // console.log(payload)
            // store.dispatch('updateAdminByUser',payload)
            that.$router.push("/admin")
          }else {
            // store.dispatch('updateStudentByUser',payload)
